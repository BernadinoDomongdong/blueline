...
- **Add one source at a time, without starting over.** Drag a source
  chip straight from the Sources list onto the canvas to trace and
  merge in just that source — existing nodes and edges are left
  exactly as they are. Unlike **Infer lineage** (which retraces
  everything and always asks first before it would overwrite manual
  work), this never replaces anything, so there's nothing to confirm.
- **Draws it like a blueprint.** High-confidence lineage (including
  ...
  and what it feeds; click a table or view to expand it in place into
  a list of its own traced columns, and click it again to collapse it
  back down.
