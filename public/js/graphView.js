...
// Cap on how many columns an expanded table/view node lists inline —
// a table with a very large traced column set would otherwise produce
// an unreadably tall node; see _nodeLabel / _columnsForTable.
const EXPANDED_COLUMN_LIMIT = 60;
...
export class GraphView {
  ...
  /** Keeps cytoscape's canvas in sync with its container's actual size — needed for fullscreen mode, sidebar collapse, and plain window resizing, none of which cytoscape detects on its own. */
  _setupResizeObserver() {
    if (typeof ResizeObserver === 'undefined') return; // ancient browser — degrade quietly, manual fit() still works
  ...
  _wireEvents() {
    this.cy.on('tap', 'node', (evt) => {
      ...
      if (this._connectModeActive) {
        ...
        this._toggleTableExpansion(evt.target);
      }
   ...
   * column-typed nodes share that id prefix. Used by _nodeLabel to
   * render an expanded table's column list inline on the canvas.
   */
  _columnsForTable(tableId) {
    if (!this.cy) return [];
    const prefix = `${tableId}.`;
    return this.cy.nodes('[type = "column"]').filter((n) => n.id().startsWith(prefix));
  }

  /**
   * Click a table/view node to expand it in place into a list of its
   * own columns (collapsing back on a second click). A no-op for
   * nodes with no traced columns, or for column/measure nodes
   * themselves — there's nothing useful to expand into.
   */
  _toggleTableExpansion(node) {
    const type = node.data('type');
    if (type !== 'table' && type !== 'view') return;
    if (this._columnsForTable(node.id()).length === 0) return;
    node.data('_expanded', !node.data('_expanded'));
    this.cy.style().update();
  }

  /** Computed node label — table/view nodes show their column list inline while expanded; every other node just shows its plain label. */
  _nodeLabel(el) {
    const label = el.data('label') || el.id();
    const type = el.data('type');
    if (type !== 'table' && type !== 'view') return label;
    const columns = this._columnsForTable(el.id());
    if (columns.length === 0) return label;
    if (!el.data('_expanded')) return `▸ ${label}`;
    const names = columns.map((c) => c.data('label') || c.id()).sort((a, b) => a.localeCompare(b));
    const shown = names.slice(0, EXPANDED_COLUMN_LIMIT);
    const lines = [`▾ ${label}`, '─'.repeat(Math.min(28, label.length + 2)), ...shown];
  ...
  hasEdgeBetween(source, target) {
    ...
    return this.cy.edges().some((e) => e.data('source') === source && e.data('target') === target);
  }
   ...
   */
  /**
   * A node's `_expanded` flag (see _toggleTableExpansion) is pure
   * canvas view-state, not part of the lineage graph itself — strip
   * it here so it never leaks into app state, JSON exports, or
   * anywhere else this snapshot ends up.
   */
  getGraphSnapshot() {
    if (!this.cy) return { nodes: [], edges: [] };
    const nodes = this.cy.nodes().map((n) => {
      const { _expanded, ...data } = n.data();
    ...
    return [
      {
        ...
        style: {
          ...
          label: (el) => this._nodeLabel(el),
          'font-family': 'IBM Plex Mono, monospace',
          'font-size': 11,
          color: c.ink,
          'text-valign': 'center',
          'text-halign': 'center',
          'text-wrap': (el) => (el.data('_expanded') ? 'wrap' : 'ellipsis'),
          'text-max-width': (el) => (el.data('_expanded') ? '220px' : '110px'),
          'text-justification': (el) => (el.data('_expanded') ? 'left' : 'center'),
          width: 'label',
          height: (el) => (el.data('_expanded') ? 'label' : 34),
          padding: (el) => (el.data('_expanded') ? '12px' : '10px'),
          'background-color': c.panelRaised,
          'border-width': 2,
          'border-color': (el) => c[el.data('confidence')] || c.high,
          'z-index': (el) => (el.data('_expanded') ? 10 : 1),
        },
      },
      {
