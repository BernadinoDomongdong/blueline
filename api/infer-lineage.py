...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        # Everything below is one request's worth of work, wrapped in
        # a single outer guard: a request-handling bug in ANY of it
        # (a socket read, decoding, validation, extraction) must never
        # surface as an opaque platform-level crash — that's a real
        # failure mode this had before: a "Request failed (500)" with
        # no message at all reaching the browser, because the
        # exception happened outside the two narrow try/excepts this
        # method used to have. Every branch now returns clean,
        # specific JSON, always.
        try:
            self._handle_post()
        except Exception as ex:
            self._send_json(500, {'error': f'Unexpected server error ({type(ex).__name__}): {ex}'})
    ...
    def _handle_post(self):
        ...
        # Read exactly the declared size (already capped above), never
        # more — reading MAX_BODY_BYTES+1 regardless of a small
        # declared_length would make this block waiting for bytes the
        # client never sends.
