 ...
 *     structured-JSON extraction workload, surfaced in the UI's model
 *     picker (see api/models.js) — any other OpenRouter model id also
 *     works from .env, just not selectable live in the picker.
...
// windows (SSAS query sets can be long). This is a curated, static
// list on purpose (not a live catalog fetch): it's what the UI's model
// picker offers, so it has to be a fixed, known-safe set the server
// can validate a client's selection against — see sanitizeModelChoice.
// OpenRouter's free-model roster in particular rotates often; if a
// ":free" id below ever 404s, browse https://openrouter.ai/models?q=free
// for its current replacement and update the entry below.
const FREE_MODELS = [
  {
    id: 'poolside/laguna-m.1:free',
    label: 'Poolside Laguna M.1 (free)',
    note: 'Best free pick — a coding-agent model, good at turning SQL/DAX into structured JSON.',
    contextLength: 256000,
  },
  {
    id: 'tencent/hy3:free',
    label: 'Tencent HY3 (free)',
    note: 'Large (295B) general reasoning model, solid fallback.',
    contextLength: 262000,
  },
  {
    id: 'nvidia/nemotron-3-ultra-550b-a55b:free',
    label: 'NVIDIA Nemotron 3 Ultra (free)',
    note: 'Frontier-scale reasoning, 1M context — for very large source-query sets.',
    contextLength: 1000000,
  },
  {
    id: 'poolside/laguna-xs-2.1:free',
    label: 'Poolside Laguna XS 2.1 (free)',
    note: 'Smaller/faster coding model if you hit the rate limit on the one above.',
...
const PAID_MODELS = [
  {
    ...
    note: "Recommended default — the model this app's prompts were originally written for.",
    contextLength: 1000000,
  },
  {
    id: 'anthropic/claude-opus-4.8',
    label: 'Claude Opus 4.8',
    note: "Anthropic's most capable — worth it for large or messy source sets.",
    contextLength: 500000,
  },
  {
    id: 'openai/gpt-5.6-sol',
    label: 'GPT-5.6 Sol',
    note: "OpenAI's flagship — strong coding/reasoning, 1M context.",
    contextLength: 1000000,
  },
  {
    id: 'google/gemini-3.5-flash',
    label: 'Gemini 3.5 Flash',
    note: 'Near-flagship coding quality for a fraction of the cost, 1M context.',
    contextLength: 1000000,
  },
  {
    id: 'z-ai/glm-5.2',
    label: 'GLM 5.2',
    note: 'Cheapest of this list with a 1M context window.',
    contextLength: 1000000,
  },
];
const DEFAULT_OPENROUTER_MODEL = FREE_MODELS[0].id;
const ALL_CURATED_MODELS = [...FREE_MODELS, ...PAID_MODELS];
...
/**
 * Validates a model id a client requested against the curated
 * allowlist above — never trust a client-supplied string as a literal
 * upstream model id, since it directly determines which provider
 * model gets billed. Falls back to the deployment default (silently,
 * not an error) for anything unrecognized or, when paid models are
 * disabled, anything from PAID_MODELS.
 * @param {string|undefined|null} requestedId
 * @returns {string} a known-safe OpenRouter model id
 */
function sanitizeModelChoice(requestedId) {
  if (!requestedId) return DEFAULT_OPENROUTER_MODEL;
  const match = ALL_CURATED_MODELS.find((m) => m.id === requestedId);
  if (!match) return DEFAULT_OPENROUTER_MODEL;
  const isPaid = PAID_MODELS.some((m) => m.id === match.id);
  if (isPaid && !paidModelsAllowed()) return DEFAULT_OPENROUTER_MODEL;
  return match.id;
}

/**
 * The data behind the UI's model picker (api/models.js). Static and
 * synchronous — no upstream call — so the picker renders instantly.
 * @returns {{ provider: string, free: Array, paid: Array, allowPaid: boolean, default: string }}
 */
function getModelCatalog() {
  return {
    provider: (process.env.LLM_PROVIDER || 'openrouter').toLowerCase().trim(),
    free: FREE_MODELS,
    paid: PAID_MODELS,
...
async function callOpenRouter({ system, messages, maxTokens, temperature, model: requestedModel }) {
  ...
  const model = requestedModel ? sanitizeModelChoice(requestedModel) : process.env.OPENROUTER_MODEL || DEFAULT_OPENROUTER_MODEL;
